...
# NumberOfEvents(BinEdges, DetectorAcceptance, Flux, Days)
# Inputs:
...
#       Days = number of days of livetime
#
#Output:
#       N = number of event the detector will see in each energy bin assuming the given flux 
#
...
# importlib.reload(sys.modules['NumberOfEventsFunction'])
...
def NumberOfEvents(BinEdges, DetectorAcceptance, Flux, Days): # function name and inputs
    ...
    for x in range(0,NumberOfBins):
        BinWidth=(BinEdges[x+1]-BinEdges[x])
        
        # First performs trapazoid rule on acceptance data and store it
        acc = np.trapz([DetectorAcceptance[x],DetectorAcceptance[x+1]], x=[BinEdges[x],BinEdges[x+1]])/(BinWidth)
        #print(acc)
        Int_Acc.append(acc)
        
        # Next performs trapazoid rule on flux data and stores it
        fluxint = np.trapz([Flux[x],Flux[x+1]], x=[BinEdges[x],BinEdges[x+1]])/(BinWidth)
        #print(fluxint)
        Int_Flux.append(fluxint)
    
        # Finally, the loop calculates the number of events in the bin
        #print(Int_Flux[x]*SecPerYear*Int_Acc[x]/((BinEdges[x]+BinEdges[x+1])/2))
        N.append(Int_Flux[x]*SecPerDay*Days*Int_Acc[x]*(BinWidth))
